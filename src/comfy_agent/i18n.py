...
_T: dict[str, tuple[str, str]] = {
    ...
    "judge.manual": ("Manual prompt (no LLM, no Jev)", "手動 prompt（不用 LLM、不用 Jev）"),
    ...
    "ui.judge": ("Jev review (or manual prompt)", "Jev 評審（或手動 prompt）"),
    "ui.manual_pos": ("Positive prompt (sent as written)", "Positive prompt（照原樣送出）"),
    "ui.manual_neg": ("Negative prompt (sent as written)", "Negative prompt（照原樣送出）"),
    ...
    "msg.need_positive": ("Enter a positive prompt", "請輸入 positive prompt"),
    ...
    "msg.manual": ("Manual: sending the prompt to ComfyUI as written", "手動：照原樣把 prompt 送到 ComfyUI"),
    ...
    "report.manual": ("Manual prompt (no LLM, no Jev)", "手動 prompt（不用 LLM、不用 Jev）"),
