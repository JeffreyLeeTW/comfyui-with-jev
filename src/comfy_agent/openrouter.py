...
class PromptWriter:
    ...
    def _chat(self, model: str, messages: list[dict]) -> tuple[str, str | None]:
        """Return (reply text, finish reason)."""
        raise NotImplementedError
    ...
    def generate(
        ...
        raise LLMError(f"Model did not return valid prompt JSON: {last_err}")


class OpenRouterClient(PromptWriter):
    name = "openrouter"
    ...
    def list_models(self, need_vision: bool = False) -> list[ModelInfo]:
        """Free models only (prompt and completion price 0)."""
        return self.list_free_models(need_vision)

    def list_free_models(self, need_vision: bool = False) -> list[ModelInfo]:
