...
def test_run_report(settings):
    ...
    assert "http://x/view" in html and "第 2 輪通過" in html and "SFW" in html
...
def test_compare_report_has_both_arms_and_delta(settings):
    ...
    assert "有 Jev" in html and "無 Jev（第 1 版）" in html and "分數差異" in html
...
def test_report_handles_refusal_and_old_logs(settings, tmp_path):
    ...
    assert "模型拒絕" in export_report(r.log_path).read_text()
